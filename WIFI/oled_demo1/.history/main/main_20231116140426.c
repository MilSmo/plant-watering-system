...
#define WEB_SERVER "agh.edu.pl"
#define WEB_PORT "80"
#define WEB_PATH "/"
#define WEB_PAGE "https://agh.edu.pl"
...
// ... (http_event_handler function remains the same)

void http_get_page(const char *url, const char *request) {
    esp_http_client_config_t config = {
        .url = url,
        .event_handler = http_event_handler,
        .transport_type = HTTP_TRANSPORT_OVER_SSL,
    ...
    // Use the provided request string
...
static const char *TAG_HTTP_TASK = "HTTP_TASK";
...
static void http_request_task(void *pvParameters) {
    while (1) {
        ...
        ESP_LOGI(TAG_HTTP_TASK, "Internet connection is available. Performing HTTP request.");

        // Perform the HTTP request here
        http_get_page("https://kis.agh.edu.pl", REQUEST);

        // You can add more logic or perform additional HTTP requests if needed

        vTaskDelay(60000 / portTICK_PERIOD_MS);  // Delay before the next HTTP request (e.g., 1 minute)
    }
}
...
void app_main(void)
{
    ...
    // const char *url = "https://www.agh.edu.pl/";
    ...
    // }
    // if (wifi_status) {
    //     // Use the HTTP client to fetch the page with the provided REQUEST string
    //     http_get_page("https://www.agh.edu.pl/", REQUEST);

    //     // You can also use the previous HTTP request code here
    //     // by replacing the URL and any other necessary parameters.
    // }
