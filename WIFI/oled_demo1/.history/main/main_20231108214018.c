...
oid http_get_task(void *pvParameters)
{
    while(1) {
        ...
        if (err == ESP_OK) {
            ...
            while (content_length > 0) {
                ...
                printf("%.*s", read_len, buffer);
                content_length -= read_len;
            }
        } else {
            printf("HTTP GET request failed with error %d\n", err);
        }
    
        vTaskDelay(10000 / portTICK_PERIOD_MS); // Czekaj 10 sekund przed kolejnym żądaniem GET
