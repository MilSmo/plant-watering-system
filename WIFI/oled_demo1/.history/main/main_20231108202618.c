...
esp_err_t http_event_handler(esp_http_client_event_handle_t evt) {
    switch (evt->event_id) {
        case HTTP_EVENT_ON_DATA:
            ...
            parse_json_and_display_time((const char *)evt->data, &dev);
            break;
        default:
            break;
    }
...
void parse_json_and_display_time(const char *json_data) {
    cJSON *root = cJSON_Parse(json_data);
    if (root == NULL) {
        printf("Błąd podczas analizy danych JSON.\n");
        return;
    }

    cJSON *timeObject = cJSON_GetObjectItem(root, "currentDateTime");
    if (timeObject != NULL && cJSON_IsString(timeObject)) {
        const char *timeStr = timeObject->valuestring;
        printf("Czas: %s\n", timeStr);
        ssd1306_display_text(&dev, 4, timeStr, 6, false);
        
    } else {
        printf("Nie znaleziono pola 'currentDateTime' w danych JSON.\n");
    }
