...
//             uint32_t in_bitmask = 0b1;
//             uint32_t out_bitmask = 0b11;  // Zmieniłem maskę z 0b111 na 0b11
...
//                 }
//                 in_bitmask <<= 1;
//                 out_bitmask <<= 2;  // Zmieniłem maskę z 0b111 na 0b11
//             }
//         }
...
//             uint8_t image[16];  // Zmieniłem rozmiar obrazu z 24 na 16
//             for (uint8_t xx = 0; xx < 8; xx++) { // for each column (x-direction)
//                 image[xx*2+0] = 
//                 image[xx*2+1] = out_columns[xx].u8[yy];
//             }
//             if (invert) ssd1306_invert(image, 16);  // Zmieniłem rozmiar obrazu z 24 na 16
//             if (dev->_flip) ssd1306_flip(image, 16);  // Zmieniłem rozmiar obrazu z 24 na 16
//             if (dev->_address == SPIAddress) {
//                 spi_display_image(dev, page+yy, seg, image, 16);  // Zmieniłem rozmiar obrazu z 24 na 16
//             } else {
//                 i2c_display_image(dev, page+yy, seg, image, 16);  // Zmieniłem rozmiar obrazu z 24 na 16
//             }
//             memcpy(&dev->_page[page+yy]._segs[seg], image, 16);  // Zmieniłem rozmiar obrazu z 24 na 16
//         }
//         seg = seg + 16;  // Zmieniłem o 16
//     }
// }
...
void ssd1306_display_text_x2(SSD1306_t * dev, int page, char * text, int text_len, bool invert)
{
    ...
    for (uint8_t nn = 0; nn < _text_len; nn++) {
        uint8_t ch = (uint8_t)text[nn];

        if (ch == '%') {
            // Renderuj znak "%"
            uint8_t image[16];
            memset(image, invert ? 0xFF : 0x00, sizeof(image));  // Wypełnij znakiem "%" w zależności od wartości invert

            if (dev->_address == SPIAddress) {
                spi_display_image(dev, page, seg, image, 16);
            } else {
                i2c_display_image(dev, page, seg, image, 16);
            }
            memcpy(&dev->_page[page]._segs[seg], image, 16);
            seg = seg + 16;
        } else {
            // Renderuj inny znak
            uint8_t const * const in_columns = font8x8_basic_tr[ch];
            out_column_t out_columns[8];
            memset(out_columns, 0, sizeof(out_columns));
