...
void read_moisture_sensor(void *pvParameters)
{
    ...
    while (1)
    {
        ...
        // Zabezpieczenie dostępu do zmiennej globalnej
        ...
        vTaskDelay(2000 / portTICK_PERIOD_MS); // Opóźnienie między odczytami
    }
}
// MQTT
...
void send_mqtt_data(void *pvParameters)
{
    ...
    while (1)
    {
        if (xSemaphoreTake(xSemaphore, (TickType_t)10) == pdTRUE)
        {
            // Bezpieczny dostęp do global_moisture
...
void app_main(void)
{
    ...
    xTaskCreatePinnedToCore(
        display_moisture,    // Funkcja z zadaniem
        "display_moisture",  // Nazwa zadania
        2048,                // Rozmiar stosu
        &dev,                // Parametry zadania
        5,                   // Priorytet
        NULL,                // Handle do zadania
        DISPLAY_TASK_CORE_ID // Rdzeń, na którym ma działać zadanie
    );
    ...
    // xTaskCreate(&display_moisture, "display_moisture", 2048, &dev, 6, NULL);
    // xTaskCreate(&send_mqtt_data, "send_mqtt_data", 2048, NULL, 5, NULL);
