...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    // chyba to tu wysyla dane 
        ...
        msg_id = esp_mqtt_client_publish(client, "/topic/moisture", "dziala", 0, 0, 0);
        ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);
        break;
...
void display_moisture(void *pvParameters)
{
    ...
    while(1){
        ...
        char moistureStr[32];
        sprintf(moistureStr, "%d", moisture);

        // Publikacja wartości wilgotności
        int msg_id = esp_mqtt_client_publish(client, "/topic/moisture", moistureStr, 0, 0, 0);
        ESP_LOGI(TAG, "sent publish successful, msg_id=%d", msg_id);
