...
static const uint16_t character_declare_uuid = ESP_GATT_UUID_CHAR_DECLARE;
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    // }
    case ESP_GATTS_READ_EVT:
    if (param->read.handle == gatts_profile_handle_table[IDX_CHAR_VAL_A]) {
        esp_gatt_rsp_t rsp;
        memset(&rsp, 0, sizeof(esp_gatt_rsp_t));
        rsp.attr_value.handle = gatts_profile_handle_table[IDX_CHAR_VAL_A];
