...
#define GATTS_TAG "GATTS_BATTERY"
...
#define SAMPLE_DEVICE_NAME          "ESP_Battery_Server"

#define GATTS_DEMO_CHAR_VAL_LEN     0x40

static uint8_t adv_service_uuid128[32] = {
    // First UUID, 16-bit, [12], [13] is the value
    0xfb, 0x34, 0x9b, 0x5f, 0x80, 0x00, 0x00, 0x80, 0x00, 0x10, 0x00, 0x00, 0xEE, 0x00, 0x00, 0x00,
    // Second UUID, 32-bit, [12], [13], [14], [15] is the value
    0xfb, 0x34, 0x9b, 0x5f, 0x80, 0x00, 0x00, 0x80, 0x00, 0x10, 0x00, 0x00, 0xFF, 0x00, 0x00, 0x00,
    // Third UUID, 16-bit, [12], [13] is the value
    0x0F, 0x18, // 0x180F in little-endian format
    // Fourth UUID (for Battery Level Characteristic), 16-bit, [12], [13] is the value
    0x19, 0x2A, // 0x2A19 in little-endian format
};

static uint8_t battery_level = 50;  // Example: 50% battery level
...
void gatts_battery_service_init() {
    esp_err_t ret;

    esp_bt_controller_config_t bt_cfg = BT_CONTROLLER_INIT_CONFIG_DEFAULT();
    esp_err_t bt_cfg_ret = esp_bt_controller_init(&bt_cfg);
    if (bt_cfg_ret) {
        ESP_LOGE(GATTS_TAG, "initialize controller failed\n");
        return;
    }
    ...
    esp_ble_gatts_cb_register(gatts_profile_event_handler);

    // Battery Service
    esp_ble_gatts_incl_svc_t battery_incl_svc = {
        .service_uuid.len = ESP_UUID_LEN_16,
        .service_uuid.uuid.uuid16 = GATTS_SERVICE_UUID_BATTERY,
    };

    esp_ble_gatts_char_val_t battery_char_val = {
        .handle = 0x0003,
        .offset = 0,
        .len = 1,
        .value = &battery_level,
    ...
    ctrl.auto_rsp = ESP_GATT_RSP_BY_APP;

    esp_ble_gatts_char_t battery_char = {
        .char_val = &battery_char_val,
    ...
    esp_ble_gatts_db_t battery_db = {
        .db_size = sizeof(battery_db),
        .attr_tab = {
            [0] = {
                .attr_control = NULL,
                .att_desc = NULL,
            ...
            [3] = {
                .p_incl_svc = &battery_incl_svc,
            },
        },
    };

    ret = esp_ble_gatts_app_register(ESP_APP_ID, &battery_db);
    if (ret) {
        ESP_LOGE(GATTS_TAG, "gatts app register failed, error code = %x", ret);
        return;
    }

    esp_err_t create_attr_ret = esp_ble_gatts_create_attr_tab(battery_db.attr_tab, gatts_if, sizeof(battery_db.attr_tab) / sizeof(battery_db.attr_tab[0]), 0);
    if (create_attr_ret) {
