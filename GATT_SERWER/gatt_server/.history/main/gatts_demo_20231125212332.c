...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_READ_EVT:
    {
        ...
        //                         ESP_GATT_OK, &rsp);
        gl_profile_tab[PROFILE_A_APP_ID].battery_level = 0x55;
        ESP_LOGI(GATTS_TAG, "Battery Level: %u", gl_profile_tab[PROFILE_A_APP_ID].battery_level);

        // Konwertuj wartość battery_level na string
        char battery_level_str[4]; // Dla bezpieczeństwa używamy 4 bajtów na string (maksymalnie 3 cyfry + znak null)
        ...
        memcpy(rsp.attr_value.value, battery_level_str, strlen(battery_level_str));
        rsp.attr_value.value[strlen(battery_level_str)] = '\0'; // Dodaj znak null na końcu
