...
// 
esp_ble_gatts_add_service(gatts_if, &service_uuid, GATTS_NUM_HANDLE);
esp_ble_gatts_add_char(gatts_if, &service_uuid, &char_uuid, &char_prop, &char_value, &gatts_attr_control);
esp_ble_gatts_add_char_descr(gatts_if, &descr_uuid, &descr_value, gatts_attr_control.handle, NULL, ESP_GATT_PERM_READ_ENCRYPTED | ESP_GATT_PERM_WRITE_ENCRYPTED);
...
static void gatts_profile_b_event_handler(esp_gatts_cb_event_t event, esp_gatt_if_t gatts_if, esp_ble_gatts_cb_param_t *param)
{
    ...
    case ESP_GATTS_WRITE_EVT:
    {
        if (param->write.len == 1 && param->write.handle == gatts_profile_handle_table[IDX_CHAR_VAL_A])
        {
            battery_level = param->write.value[0];
            // Tutaj możesz dodać kod do obsługi zmiany poziomu naładowania baterii
            // Na przykład, możesz przechowywać wartość battery_level i używać jej w innych częściach kodu
        }
        break;
